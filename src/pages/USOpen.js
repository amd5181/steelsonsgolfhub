...
export default function USOpenLeaderboard() {
  ...
  const previousUsOpenData = useRef([]);

  const API_KEY = "AIzaSyC-0Zrg5OARvAqSmyK8P8lkJqVCccGjrF4";
  const SPREADSHEET_ID = "1Nx_8Mkf3U14civkKD5b5sgXL8yVpErC1g-9rC1Z1hbM";
  const RANGE = "US Open Leaderboard!A1:Z1000";
  const encodedRange = encodeURIComponent(RANGE);

  const tournamentStart = new Date("2025-06-12T10:00:00-04:00");
  const now = new Date();
  ...
  const fetchData = () => {
    ...
    fetch(url, { cache: "no-store" })
      ...
      .then((data) => {
        ...
        const newUsOpen = rows.slice(1, 12).map((r) => r.slice(17, 19));
        ...
        if (!isEqual(previousUsOpenData.current, newUsOpen)) {
          previousUsOpenData.current = newUsOpen;
          setUsOpenData(newUsOpen);
        }
